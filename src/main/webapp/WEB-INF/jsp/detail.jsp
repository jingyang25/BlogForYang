...
<head>
    ...
    <style>
        ...
        *{
        ...
        #bg {
            position: relative;
            background: url("/static/img/296494-106.jpg") no-repeat gray;
...
<article class="article">
    ...
    <div style="position: relative;left: 12%">
        <c:if test="${!empty lastBlog }">
            <div ><a href="/blog/?id=${lastArticle.id}"><h4><span class="label label-primary">上一篇:${lastBlog.title}</span></h4></a></div>
        </c:if>
        <c:if test="${!empty nextBlog }">
            <div><a href="/article/?id=${nextArticle.id}"><h4><span class="label label-success">下一篇:${nextBlog.title}</span></h4></a></div>
        </c:if>
    </div>
    </div>
